...
```bash
# Register the app
autoserve register my-server.yml

# Start the app
autoserve up my-server

# Check status
autoserve status my-server

# Scale manually
autoserve scale my-server 3

# Stop the app
autoserve down my-server
```
...
response = requests.post("http://127.0.0.1:8000/apps/register", json=spec)
print(response.json())
```
...
autoserve register test/my-server.yml
autoserve up my-server
```
